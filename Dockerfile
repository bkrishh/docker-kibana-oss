...
RUN apk --update add bash curl wget && \
    mkdir /opt && \
    curl -s https://artifacts.elastic.co/downloads/kibana/kibana-oss-${KIBANA_VERSION}-linux-x86_64.tar.gz | tar zx -C /opt && \
    apk add nodejs && \
    ...
    ln -sf /usr/bin/node /opt/kibana-${KIBANA_VERSION}-linux-x86_64/node/bin/node && \
		/opt/kibana-${KIBANA_VERSION}-linux-x86_64/bin/kibana-plugin install --no-optimize "https://oss.sonatype.org/content/repositories/releases/com/floragunn/search-guard-kibana-plugin/$SG_VERSION/search-guard-kibana-plugin-$SG_VERSION.zip" && \
    rm -rf /var/cache/apk/*
...
RUN /opt/kibana-${KIBANA_VERSION}-linux-x86_64/bin/kibana --optimize
